...
def test_remove_from_list(): 
    # je teste que j'arrive à enlever un élément qui est bien dans ma liste
    response = client.delete("/liste", params={"element": 5}) 
    assert response.status_code == 200 
    assert response.json() == {"content": []} 

    # je teste que quand je donne autre chose qu'integer j'obtiens une erreur lié au validateur (erreur 422)
